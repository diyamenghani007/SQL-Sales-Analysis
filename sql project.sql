...
with cte as
(select format(order_date, 'yyyyMM') as order_year_month,  sum(sale_price) as sales, category from df_orders 
...
from df_orders group by sub_category, year(order_date)
), cte2 as (
select sub_category, 
sum( case when order_year =  2022 then sales else 0 end) as sales_2022,
sum( case when order_year =  2023 then sales else 0 end) as sales_2023
...
select *
, (sales_2023 - sales_2022) from cte2
